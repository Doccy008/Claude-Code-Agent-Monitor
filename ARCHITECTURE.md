...
> **Cursor is a first-class stored provider.** Compatible live hook events are classified from their `.cursor` transcript path. `server/lib/cursor-ingest.js` also scans `~/.cursor/projects/*/agent-transcripts`, joins `~/.cursor/chats` metadata, repairs older hook-created placeholders, imports subagents, and snapshots JSONL under `<dashboard-data>/cursor-transcripts`. Conversation routes resolve the live file first and the snapshot second. The product selector intentionally groups Cursor under the Claude Code scope, while storage and pricing retain `provider = 'cursor'` so billing never crosses rate cards.
...
| `server/lib/cursor-ingest.js` | Fingerprints Cursor main/subagent transcripts and chat metadata, imports or repairs `provider='cursor'` rows, backfills native titles/cwd/prompt/turn context, and snapshots JSONL before Cursor cleanup. The `DASHBOARD_CURSOR_SYNC_MS` poll skips unchanged sessions and emits the ordinary session/agent WebSocket frames for changed rows. |
