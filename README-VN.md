...
| `DASHBOARD_TOKEN_REPAIR` | `1` (bật) | Tự động sửa một lần các tổng Token lịch sử bị ảnh hưởng bởi việc cộng phồng theo từng bản ghi hoặc bỏ sót subagent phẳng cùng model. Bản sửa trực tiếp không thể xử lý lại mọi phiên đã hoàn tất, và `replaceTokenUsage` không thể hạ mốc cao cũ. Chạy một lần cho mỗi cơ sở dữ liệu (có marker, hoãn khỏi đường khởi động, bỏ qua khi có dashboard khác dùng chung thư mục dữ liệu) và sao lưu các hàng cũ sang `token_usage_pre_repair` trước. Đặt `0` để bỏ qua và tự chạy `npm run repair-tokens` |
