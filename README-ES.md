...
Cursor no necesita una opción de configuración separada: elegir **Claude Code** en la pantalla inicial también habilita Cursor. El panel explora `~/.cursor/projects/*/agent-transcripts`, une metadatos de `~/.cursor/chats` y completa sesiones existentes con títulos, proyectos, prompts, turnos y subagentes. Los JSONL principales y de subagentes se guardan en el directorio de datos del panel, por lo que Conversation sigue disponible después de que Cursor limpie el historial original.

Las tarjetas muestran `Cursor · <título>` y subtítulos de proyecto, turnos y subagentes. La tabla editable **Precios de Cursor** cubre Grok, Composer y el catálogo publicado de terceros sin reutilizar tarifas de Claude o Codex. `DASHBOARD_CURSOR_HOME` y `DASHBOARD_CURSOR_SYNC_MS` permiten ajustar la ruta y el sondeo.
...
| `DASHBOARD_CURSOR_SYNC_MS` | `5000` | Intervalo de seguridad (ms) para descubrir historial de Cursor mediante huellas. Los Hooks compatibles siguen ingresándose de inmediato; `0` desactiva el escaneo periódico. |
